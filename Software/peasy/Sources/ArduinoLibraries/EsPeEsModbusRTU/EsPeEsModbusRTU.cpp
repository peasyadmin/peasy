...
EsPeEsModbusSlave::EsPeEsModbusSlave(uint8_t slave) {
    if (slave >= 0 & slave <= 247) {
	_slave = slave;
    }
}

void EsPeEsModbusSlave::setup(long baud) {
    Serial.begin(baud);
}
...
static int receive(uint8_t *req, uint8_t _slave)
{
	...
	/* The timeout is defined to ~10 ms between each bytes.  Precision is
	   ...
	   principle (millis overflows after 50 days, etc) */
		...
		delayMicroseconds(MODBUS_TIMEOUT_MICROSECONDS);
		if (++i == MODBUS_TIMEOUT_RETRIES) {
		    /* Too late, bye */
		    return -3;
		}
	    }
...
int EsPeEsModbusSlave::loop(uint16_t* tab_reg, uint16_t nb_reg, char averageCycleIndex, char cycleIndex, char timeoutIndex, char errorIndex)
{
    ...
    if (Serial.available()) {
        _currentError = receive(req, _slave);
        if ((_currentError < 0) && (_currentError > -3)) {
            _totalErrors = _totalErrors + 1;
            tab_reg[errorIndex] = _totalErrors;
        }
        if (_currentError == -3) {
            _totalTimeouts = _totalTimeouts + 1;
            tab_reg[timeoutIndex] = _totalTimeouts;
        }
        
        if (_currentError > 0) {
            reply(tab_reg, nb_reg, req, _currentError, _slave);
            _currentCycleTime = micros() - _lastCycleTime;
            _averageFullValue =  _averageFullValue + _currentCycleTime;
            _averageCounter = _averageCounter + 1;
            tab_reg[cycleIndex] = _currentCycleTime;
            tab_reg[averageCycleIndex] = _averageFullValue / _averageCounter;
            writeOutput();
        }
    }
    else {
